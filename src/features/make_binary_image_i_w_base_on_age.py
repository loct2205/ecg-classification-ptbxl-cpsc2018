...
base_folder = 'H:/Output/image_base_on_age'
output_folder = 'H:/Output/binary_images_i_w_base_on_age_output'
    ...
    for img_idx, files in images_by_idx.items():
        if len(files) == 12:
            ...
            for file in files:
                ...
                # Kiểm tra và thay đổi kích thước ảnh về 900x600 (width x height)
                img = img.resize((900, 600))  # Resize theo width x height
            ...
            # Chuyển danh sách ảnh thành ma trận numpy (12, 900, 600, 4)
            images_array = np.array(images)  # Kích thước: (12, 900, 600, 4)
