...
output_folder = 'H:/Output/find_peaks_i_w_output'
        ...
        try:
            ...
            with open(csv_filename, mode='w', newline='') as csvfile:
                ...
                for signal_idx, ecg_signal in enumerate(signals):
                    ...
                    # Xử lý từng khoảng i
                    for i in range(1, len(r_positions) - w + 1):
                        selected_peaks = r_positions[i - 1:i - 1 + w]
                        row = [f"Interval {i}"] + list(selected_peaks)
                        csvwriter.writerow(row)

            print(f"Xử lý xong tệp: {filename}, lưu tại: {csv_filename}")
