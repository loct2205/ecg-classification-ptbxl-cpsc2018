...
output_folder = 'H:/Image'

# Tạo thư mục đích nếu chưa tồn tại
os.makedirs(output_folder, exist_ok=True)

# Hàm chia các nhịp tim thành nhóm 5 nhịp để vẽ
def group_heartbeats(peaks, group_size=5):
    num_peaks = len(peaks)
    groups = []

    # Nếu số đỉnh >= 13, chia thành các nhóm liên tiếp
    if num_peaks >= 13:
        for i in range(num_peaks - group_size + 1):  # Đảm bảo có đủ đỉnh để tạo nhóm
            group = peaks[i:i + group_size]
            groups.append(group)
    else:
        # Nếu số đỉnh < 13, chia theo kiểu xoay vòng
        for i in range(num_peaks):  # Lặp qua từng đỉnh
